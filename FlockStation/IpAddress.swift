...
public protocol IpAddressRangeBuildable {
  func build(ifaddrs_ptr : UnsafeMutablePointer<ifaddrs>) -> IpAddressRangable?
}

public class IpAddressRangeBuilder : IpAddressRangeBuildable {
  internal static let _builder = IpAddressRangeBuilder()
  
  public func build(ifaddrs_ptr : UnsafeMutablePointer<ifaddrs>) -> IpAddressRangable? {
    let flags = Int32(ifaddrs_ptr.memory.ifa_flags)
    var sa_addr = ifaddrs_ptr.memory.ifa_addr.memory
    let addr = UnsafeMutablePointer<sockaddr_in>(ifaddrs_ptr.memory.ifa_addr)
    ...
    if (flags & (IFF_UP|IFF_RUNNING|IFF_LOOPBACK)) == (IFF_UP|IFF_RUNNING) {
      ...
      if let ipAddress = builder.build(ipAddress), netmaskAddress = builder.build(netmaskString) {
        return IpAddressRangeFactory.factory.range(fromIpAddress: ipAddress, netmask: netmaskAddress)
      } else {
        return nil
      }
    }
...
extension IpAddressRangeBuildable {
  static var builder:IpAddressRangeBuildable {
    return IpAddressRangeBuilder._builder
  }
}
